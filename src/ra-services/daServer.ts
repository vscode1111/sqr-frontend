...
import { createUpdateData, deleteRecord } from "./utils";
...
export function daServer(apiUrl: string): DataProvider {
  return {
    ...
    update: async (resource, params) => {
      const data = (await createUpdateData(params)) as ContractDTO;
      console.log(111, params.id, data);

      const { json } = await httpClientToken(
